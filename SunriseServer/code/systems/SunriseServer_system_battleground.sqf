...
} forEach ("true" configClasses (missionConfigFile >> "LifeCfgWarZones" >> "SpawnZones" >> "Sunrise"));
_dp = _dpARR select 0;

_markerName = getText(missionConfigFile >> "LifeCfgWarZones" >> "SpawnZones" >> "Sunrise" >> _dp >> "markername");
_cord = selectRandom getArray(missionConfigFile >> "LifeCfgWarZones" >> "SpawnZones" >> "Sunrise" >> _dp >> "spawnPoints");
...
["BattleGround",_dpName,(_markerCord select 0),(_markerCord select 1),"BattleGround"] spawn SunriseServer_system_addWarPoint;//не изменять!
...
if (BATTLE_DEBUG) then {
	diag_log "###BattlerGround with Helicrash by Byy###";
...
["vehOpened"] remoteExecCall ["SunriseClient_warzones_message",-2];
...
["boxChargePlaced",_bombTimer] remoteExecCall ["SunriseClient_warzones_message",-2];
////////

if !(BATTLE_DEBUG) then {sleep (_bombTimer * 60);};
["boxOpened"] remoteExecCall ["SunriseClient_warzones_message",-2];
sleep 5;
...
{},
{},
{[crate] spawn SunriseClient_warzones_openBox;},
{},
[],
5,
0,
...
["BattleGround",_dpName,1234,"BattleGround"] spawn SunriseServer_system_removeWarPoint;//не изменять!