	...
	
{
	try {	
		...
		[format ["[%1] %2", "_init.sqf", "GUI script registered on variable " + format ["%1_script", configName _x]]] call BIS_fnc_log;
	} catch {
		[format ["[%1] %2", "_init.sqf", _exception]] call BIS_fnc_error;
	};
} forEach _configs;
