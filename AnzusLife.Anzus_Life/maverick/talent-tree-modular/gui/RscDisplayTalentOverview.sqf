...
try {
	...
	_updateGUI = {
		...
		if (isClass _nextLevelConfig) then {
			_ctrlTextStatusInfo ctrlSetText format ["经验值： %1/%2，可用的额外积分数： %3", life_currentExp - _currentLevelBottomExp, _currentLevelTopExp - _currentLevelBottomExp, life_currentPerkPoints];
			_ctrlProgress progressSetPosition _currentLevelProgress;
		} else {
			_ctrlTextStatusInfo ctrlSetText format ["经验值： %1，最高等级，额外奖励点数：%2", life_currentExp - _currentLevelBottomExp, life_currentPerkPoints];
	...
	switch (_mode) do
	{
		case "onLoad":
		{
			if (!life_perksInitialized) exitWith {
				["服务器仍在加载您的特权，请稍后再试","true","slow"] call ANZUS_fnc_notificationSystem;
			};
