...

{
	if (isPlayer _x && _x != player) then
	{
		...
		_list = allPlayers select {_x distance player < 300};
		uiSleep 1;
...
uiSleep 4.5;
player,format["%1 警官 向车辆人员 %2 发送了警察警告并进入战斗区域", name player, _logplayers];
uiSleep 60;
player,format["%1 警官 向车辆人员 %2 的战斗已解除不能再射击", name player, _logplayers];