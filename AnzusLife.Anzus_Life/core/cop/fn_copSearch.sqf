...
if (count _invs > 0) then {
    ...
    [0,"STR_Cop_Contraband",true,[(name _civ),[_illegal] call ANZUS_fnc_numberText]] remoteExecCall ["ANZUS_fnc_broadcast",west];
} else {
    _inv = localize "STR_Cop_NoIllegal";
};

if (!alive _civ || player distance _civ > 5) exitWith {[format[localize "STR_Cop_CouldntSearch",name _civ],true,"slow"] call ANZUS_fnc_notificationSystem;};

[parseText format["<t color='#FF0000'><t size='2'>%1</t></t><br/><t color='#FFD700'><t size='1.5'><br/>" +(localize "STR_Cop_IllegalItems")+ "</t></t><br/>%2<br/><br/><br/><br/><t color='#FF0000'>%3</t>",(name _civ),_inv,if (_robber) then {"Robbed the bank"} else {""}],true,"slow"] call ANZUS_fnc_notificationSystem;

if (_robber) then {
    [0,"STR_Cop_Robber",true,[(name _civ)]] remoteExecCall ["ANZUS_fnc_broadcast",RCLIENT];
};
