...
if (isNull _shooter OR LSNOTALIVE(_unit)) exitWith {[_unit,"tazed"] call fn_clearGlobalVar};
...
if (_shooter isKindOf "Man" && alive _unit) then {
	if !(_unit GVAR ["tazed",false]) then {
		...
		[_unit] remoteExecCall ["fn_tazeSound",RCLIENT];		
		if (NOTINVEH(_unit)) then {
			_obj = "Land_ClutterCutter_small_F" createVehicle (getPosATL _unit);
			_obj setPosATL (getPosATL _unit);
			[_unit,"AinjPfalMstpSnonWnonDf_carried_fallwc"] remoteExecCall ["fn_animSync",RCLIENT];
		...
		if (side _shooter != west OR !(license_civ_jianchag) OR !(license_civ_ztb) OR !(license_civ_faguan) OR FETCH_CONST(life_adminlevel) < 1) then {
			[[getPlayerUID _shooter,GVAR_RNAME(_shooter),"666"],"wantedAdd"] call fn_hcExec;
			if ([player] call fn_isSafeZone) then {
		...
		if (NOTINVEH(_unit)) then {
			[_unit,"amovppnemstpsraswrfldnon"] remoteExecCall ["fn_animSync",RCLIENT];
			detach _unit;
			deleteVehicle _obj;
		};
		titleText["","PLAIN"];
		[_unit,"tazed"] call fn_clearGlobalVar;
