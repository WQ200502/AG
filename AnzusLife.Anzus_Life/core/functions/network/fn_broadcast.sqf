    ...
    if (_type isEqualType []) then {
        {
            switch (_x) do {
                ...
                case 1: {[_msg,true,"slow"] call ANZUS_fnc_notificationSystem;};
...
if (_type isEqualType []) then {
    {
        switch (_x) do {
            ...
            case 1: {[format["%1", _message],true,"slow"] call ANZUS_fnc_notificationSystem;};
            case 2: {titleText[format ["%1",_message],"PLAIN"];};
            case 3: {[parseText format ["%1", _message],true,"slow"] call ANZUS_fnc_notificationSystem;};
            case 5: {[parseText format["%1", _message],true,"slow"] call ANZUS_fnc_notificationSystem;};
        };
        true
    } count _type;
} else {
