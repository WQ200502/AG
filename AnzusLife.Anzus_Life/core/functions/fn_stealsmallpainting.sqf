...
if ((time - _var) < 1800) exitWith {[format["最近发生了重大犯罪, 你必须等着 %1 更多分钟.",_formatTime],true,"slow"] call ANZUS_fnc_notificationSystem;};
_oldRob = (_shop getVariable ["robbing", objNull]);
_blacklistedWeapons = MAIN_SETTINGS_TYPE(getArray,"MajorCrimes","blacklistedWeapons");
if ((west countSide playableUnits) < 4) exitWith {[format["有 %1 警察在线, 需要4警察来抢劫美术馆." ,(west countSide playableUnits)],"false","slow"] call ANZUS_fnc_notificationSystem;};
...
[format["ALARM! -美术馆被抢劫 %1", _getlocation],false,"slow"] remoteExec ["ANZUS_fnc_notificationSystem",west];
...
if(life_istazed) exitWith { ["你被打倒了!","false","slow"] call ANZUS_fnc_notificationSystem; 5 cutText ["","PLAIN"]; call ANZUS_fnc_hudUpdate; };
...
[format["You have stolen a small painting, run!!",[_kassa] call ANZUS_fnc_numberText],"true","slow"] call ANZUS_fnc_notificationSystem;
[true,"spainting",1] call ANZUS_fnc_handleInv;
