...
_nearfeiji = player nearEntities ["Air", 50];
_logplayers = [];

{
	if (isPlayer _x && _x != player) then
	{
		...
		uiSleep 1;
		[[0,2],format["%1 在50米内向你发送了打劫警告请5 秒后进行反抗! 收到的:%2人!",name player,count _list]] remoteExec ["ANZUS_fnc_broadcast",_x];
		...
		uiSleep 1;
		[[player, "pmhh",50],"ANZUS_fnc_playsound",true,false] spawn ANZUS_fnc_MP;
...
uiSleep 4.5;
player,format["%1 向飞机人员 %2 发送了打劫警告并进入战斗区域", name player, _logplayers];
uiSleep 60;
player,format["%1 向飞机人员 %2 的战斗已解除不能再射击", name player, _logplayers];