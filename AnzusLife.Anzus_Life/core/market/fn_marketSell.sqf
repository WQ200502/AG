...
_arr_resource = [_shortname] call anzus_fnc_marketGetRow;
_arr_price = [_shortname] call anzus_fnc_marketGetPriceRow;
...
[_shortname, [_shortname,_price,_globalprice,-(_modifier)], false ] call anzus_fnc_marketSetPriceRow; //dont broadcast!
...
{
	{
		if((count _x) == 2) then
		{
		    ...
		    {
			[_x select 0, _relamount, true, false] call anzus_fnc_marketBuy; //Make prices higher, no broadcast!
		    }
		    else
		    {
			_relamount = -(_relamount);
			[_x select 0, _relamount, true, false] call anzus_fnc_marketSell; //Make prices higher, no broadcast!
		    };
		}
		else
		{
