...

{
	...
	_row = [_x] call anzus_fnc_marketGetPriceRow;
	...
	{
		_text = format["%1<img size='2' image='\anzuslifecore\images\icons\trendup.paa'/> %2 $ <br/>", _text, _globalchange];
	}
	else
	{
		_text = format["%1<img size='2' image='\anzuslifecore\images\icons\trenddown.paa'/> %2 $ <br/>", _text, _globalchange];
	};
