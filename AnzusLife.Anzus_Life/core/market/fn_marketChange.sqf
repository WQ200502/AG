...
_rand = [0,200] call anzus_fnc_randomRound; //0-200
...
switch(true) do
{
	...
	{
		{
			if(random(10) <= 4) then //Random for each resource
			{
				...
				_modifier = [-20,20] call anzus_fnc_randomRound; //Verkaufte/Gekaufte Items
				...
				if(_modifier < 0) then
				{
					[_x select 0, -(_modifier), true] call anzus_fnc_marketSell; 
				}
				else
				{
					[_x select 0, _modifier, true] call anzus_fnc_marketBuy; 
				};
			};
		}
	...
	case (_rand <= 36): //6% Drogenpreis erhöhen
	{
		...
		["marijuana", [15,30] call anzus_fnc_randomRound, true] call anzus_fnc_marketBuy; 
		["cocaine_unprocessed", [15,30] call anzus_fnc_randomRound, true] call anzus_fnc_marketBuy; 
		["cocaine_processed", [15,30] call anzus_fnc_randomRound, true] call anzus_fnc_marketBuy; 
		["heroin_unprocessed", [15,30] call anzus_fnc_randomRound, true] call anzus_fnc_marketBuy; 
		["heroin_processed", [15,30] call anzus_fnc_randomRound, true] call anzus_fnc_marketBuy; 
	...
	case (_rand <= 44): //8% Ölpreis erhöhen
	{
		...
		["oil_processed", [20,40] call anzus_fnc_randomRound, true] call anzus_fnc_marketBuy; 
		
		diag_log "+Market+ Event oilp";
	};
	case (_rand <= 54): //10% Eisenpreis + zementpreis + glaspreis erhöhen
	{
		[0,format["News: cement-ironrefined-glass prices increased."]] remoteExecCall ["anzus_fnc_broadcast",RCLIENT];
		
		["cement", [20,40] call anzus_fnc_randomRound, true] call anzus_fnc_marketBuy; 
		["iron_refined", [20,30] call anzus_fnc_randomRound, true] call anzus_fnc_marketBuy; 
		["glass", [25,35] call anzus_fnc_randomRound, true] call anzus_fnc_marketBuy; 
	...
	case (_rand <= 60): //4% Goldpreis sinkt
	{
		...
		["goldbar", [15,30] call anzus_fnc_randomRound, true] call anzus_fnc_marketSell; 
	...
	case (_rand <= 68): //8% Kupfer/Silber erhöht sich
	{
		...
		["goldbar", [15,30] call anzus_fnc_randomRound, true] call anzus_fnc_marketBuy; 
		["copper_refined", [15,30] call anzus_fnc_randomRound, true] call anzus_fnc_marketBuy; 
	...
	case (_rand <= 80): //6% Drogenpreis niedriger
	{
		...
		//Erhöhe Marktpreis mit marketSell
		["marijuana", [15,30] call anzus_fnc_randomRound, true] call anzus_fnc_marketSell; 
		["cocaine_unprocessed", [15,30] call anzus_fnc_randomRound, true] call anzus_fnc_marketSell; 
		["cocaine_processed", [15,30] call anzus_fnc_randomRound, true] call anzus_fnc_marketSell; 
		["heroin_unprocessed", [15,30] call anzus_fnc_randomRound, true] call anzus_fnc_marketSell; 
		["heroin_processed", [15,30] call anzus_fnc_randomRound, true] call anzus_fnc_marketSell; 
	...
	case (_rand <= 90): //10% Schildkröten höher
	{
		...
		//Erhöhe Marktpreis mit marketSell
		["exampleitem", [15,30] call anzus_fnc_randomRound, true] call anzus_fnc_marketBuy; 	
		["exampleitem", [15,30] call anzus_fnc_randomRound, true] call anzus_fnc_marketBuy; 	
		["exampleitem", [15,30] call anzus_fnc_randomRound, true] call anzus_fnc_marketBuy; 	
		["exampleitem", [15,30] call anzus_fnc_randomRound, true] call anzus_fnc_marketBuy; 	
	...
	default //market change: default version
	{
		//diag_log format["[MARKET] marketChange did nothing. Rand=%1", _rand];
		{
			...
			if(_defaultprice > 0) then
			{
				...
				//_modifier = [-15,25] call anzus_fnc_randomRound; //Verkaufte/Gekaufte Items
