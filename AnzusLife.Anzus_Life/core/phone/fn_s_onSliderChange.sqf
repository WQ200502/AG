...
switch (_mode) do {
    case 3: {
        ...
        if (!isNil "_classname" || _classname =="") then {
            ...
            ctrlSetText[20324,format ["汽油 : %1 公升",round(_value) - (floor(_fuel * _fueltank))]];
            ctrlSetText [20323,format ["总额 : $%1",round (life_fuelPrices * (round(_value) -(floor(_fuel * _fueltank)))) ]];
        } else {
